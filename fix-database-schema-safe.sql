-- Safe database schema fix for Security Staff functionality
...
-- 2. Add missing supervisor_id column to security_staff table (if not exists)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- 3. Ensure security_staff_entities table exists with proper structure
