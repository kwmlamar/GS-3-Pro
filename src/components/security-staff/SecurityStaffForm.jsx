...
import { createSecurityStaff, updateSecurityStaff, SECURITY_STAFF_TYPES, getSecurityCompanies, getPotentialSupervisors } from '@/lib/securityStaffService';
import { getDepartments, createDepartment } from '@/lib/entityStaffService';
import { supabase } from '@/lib/supabaseClient';

const SecurityStaffForm = ({ securityStaff = null, onClose, onSuccess }) => {
  ...
  const fetchSupervisors = useCallback(async () => {
    ...
    try {
      const { data, error } = await getPotentialSupervisors(securityStaff?.id);
      if (error) throw error;
      setSupervisors(data || []);
    } catch (error) {
  ...
  useEffect(() => {
    if (securityStaff) {
      setFormData({
        ...
        type: securityStaff.type || '',
        entities: securityStaff.entities?.map(e => e.site_id) || [],
        department: securityStaff.departments?.name || '',
        status: securityStaff.status || 'Active',
        compliance: securityStaff.compliance || 100,
        certifications: Array.isArray(securityStaff.certifications) 
          ? securityStaff.certifications.join(', ') 
          : securityStaff.certifications || '',
        ...
        supervisor_ids: securityStaff.supervisor_ids || [securityStaff.supervisor_id].filter(Boolean) || [],
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Split name into first_name and last_name for security_staff table
      ...
      const securityStaffData = {
        first_name,
        last_name,
        employee_id: `SEC${Date.now()}`, // Generate unique employee ID
        position: formData.role,
        status: formData.status,
        hire_date: formData.hire_date,
        email: formData.email,
        phone: formData.phone,
        compliance_score: formData.compliance,
      ...
      let result;
      if (isEditing) {
        result = await updateSecurityStaff(securityStaff.id, securityStaffData);
      } else {
        result = await createSecurityStaff(securityStaffData);
      }
  ...
  const securityStaffTypeOptions = Object.keys(SECURITY_STAFF_TYPES);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={formData.type} onValueChange={(value) => handleInputChange('type', value)}>
                      ...
                      <SelectContent className="bg-slate-800 border-slate-600">
                        {securityStaffTypeOptions.map((type) => (
